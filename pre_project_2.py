...
def do_all_G(nodes_n,edge_list_array,G_result,num_record,npresult,npresult_z):
    ...
    return G_result,npresult,num_record,npresult_z
#end def#####################################
def edge_switch():
    all_new_g = []
...
while not find_done:
    ########################################permutation
    G_result,npresult,num_record,npresult_z=do_all_G(nodes,edge_list_array,G_result,num_record,npresult,npresult_z)
...
for n_order in range(1,nodes):
    ...
    G_result,npresult,num_record,npresult_z=do_all_G(nodes-n_order,[i[2] for i in remove_g_all if i[2]],
                                                                G_result,num_record,npresult,npresult_z)
...
for i in range(len(all_npresult)):
    print (len(all_G_result[i]),len(all_npresult[i]),len(all_npresult_z[i]))
