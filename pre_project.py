...
def do_all_G(edge_list_array,G_result,npresult,do_times):
    ...
    ##########################################save file
        ...
        path_arr = 'output/G'+"'"*do_times+str(new_G)+'.txt'
        record_arr_file = open(path_arr, 'w')
        write_string="G"+"'"*do_times+"("+str(new_G)+"): "
        ...
        print (", check ->%d"%np.shape(np.unique(npresult,axis=0))[0])
        # print time.time()-start
        new_G+=1
...
edge_list_array=edge_list_array[:3]
...
while not find_done:
    ...
    all_new_g = []
    print "\ndo edge switch!"
    for now_G in range(num_record[-1][1],len(G_result)):
        ...
        for i in range(len(G_result[now_G])):
            ...
            for j in range(len(edges_on_node_b)):
                ...
                if len(np.where(np.all(npresult == sort(new_g), axis=(1,2)))[0]) == 0:
                    print "G"+"'"*num_record[-1][0]+"("+str(now_G-num_record[-1][1])+"): (u, v) ",
